...
export default class BoundInput extends Component {
    ...
    constructor (options) {
        ...
        switch (options.type){
            ...
            case 'date':
                ...
                break;
            case 'datetime-local':
                prototype = BoundDateTime.prototype;
                break;
        }
    ...
    loadFromRecord (defaultValue) {
        let rawValue = this.record[this.attribute]
        if (rawValue === null || rawValue === undefined) {
            rawValue = defaultValue
        }
        const value = this.load(rawValue, this.record)
...
export class BoundCheckbox extends BoundInput {
    ...
    loadFromRecord () {
        ...
        if (this.el.value == "on") {
            ...
            this.el.checked = this.load(this.record[this.attribute].toString() === this.el.value);
        }
    }
...
export class BoundDate extends BoundInput {
    ...
    initialize (options) {
        super.initialize(options);
        this.el.addEventListener('blur', this.blurInput.bind(this));
    }
...
export class BoundDateTime extends BoundInput {
