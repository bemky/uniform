...
export class SpreadsheetCell {
    ...
    renderInput () {
        ...
        if (input instanceof Promise) {
            input.then(i => {
                this.el.append(i);
                i.addEventListener('focusout', this.render.bind(this))
            })
        } else if (input) {
    ...
    async activate (event) {
        let input;
        if (event.target.matches('input, select, textarea, button') && !event.target.classList.contains('uniformSpreadsheet-cellContent')) {
            input = event.target
...
export default class Spreadsheet extends TableGrid {
    ...
    renderCell(record, model) {
        if (model.record && typeof model.record == "function") {
