...
export default class Spreadsheet extends TableGrid {
    ...
    defaultColumnRender (record, model) {
        let input;
        const value = model.load ? model.load(record[model.id]) : record[model.id]
        if (model.options) {
            input = createElement('select', Object.assign({
                content: model.options.map(option => createElement({
                    tag: 'option',
                    content: option,
            ...
            input = createElement('input', Object.assign({
                value: value
            }, model.input))
        }
